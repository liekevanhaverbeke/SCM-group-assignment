"""
VERENIGDE MOVING AVERAGE VOORSPELLING (Alle Niveaus)
- Consolideert MA voorspellingen voor: Total, City, City/Product en City/Product/Size
...
LEVELS = [
    {
        "name": "TOTAL",
        "group_cols": [],
        "output_xlsx": "Forecast/moving_average_total.xlsx"
    },
    {
        "name": "CITY",
        "group_cols": ["channel_id"],
        "output_xlsx": "Forecast/moving_average_city.xlsx"
    },
    {
        "name": "CITY_PRODUCT",
        "group_cols": ["channel_id", "product_id"],
        "output_xlsx": "Forecast/moving_average_city_product.xlsx"
    },
    {
        "name": "CITY_PRODUCT_SIZE",
        "group_cols": ["channel_id", "product_id", "size"],
        "output_xlsx": "Forecast/moving_average_city_product_size.xlsx"
    }
]

# ── 2. Moving Average Hulpfunctie ───────────────────────────────────────────
N = 3

def ma_forecast(waarden, n=N):
    if len(waarden) < n:
        return float(np.mean(waarden)) if len(waarden) > 0 else 0.0
...
def run_level(df, config):
    ...
    print(f"\n" + "="*80)
    print(f"VERWERKEN: {level_name} (Moving Average)")
    print("="*80)
    ...
    for i, (_, row) in enumerate(combinaties.iterrows()):
        ...
        if not train_val.empty and not actual_val_row.empty:
            pred_2025 = ma_forecast(train_val["true_demand"].tolist())
        ...
        # Forecast 2026
        if not subset.empty:
            pred_2026 = ma_forecast(subset["true_demand"].tolist())
