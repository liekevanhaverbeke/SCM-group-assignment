"""
Vraagvoorspelling 2026 via Simple Exponential Smoothing
...
- SES modelleert enkel het niveau (geen trend) → geschikt voor stabiele of licht fluctuerende vraag
"""
...
# ── 2. Hulpfunctie: Simple Exponential Smoothing ─────────────────────────────
def ses_forecast(waarden, alpha=None, predict_steps=1):
    """
    Simple Exponential Smoothing: model met enkel niveau.
    Indien alpha niet opgegeven, wordt deze geoptimaliseerd.
    """
    ...
    alpha_val = fit.model.params["smoothing_level"]
    return float(forecast), alpha_val

# ── 3. VALIDATIE: train 2018–2024, voorspel 2025 ────────────────────────────
print("\n" + "=" * 65)
print("VALIDATIE – Voorspelling 2025 vs. Actuals (SES)")
print("=" * 65)
...
for _, row in combinaties.iterrows():
    ...
    pred_2025, alpha_val = ses_forecast(train[demand_col].tolist(), predict_steps=1)
...
print("\n" + "=" * 65)
print("FORECAST – Voorspelling 2026 (SES)")
print("=" * 65)
...
for _, row in combinaties.iterrows():
    ...
    pred_2026, alpha_val = ses_forecast(subset[demand_col].tolist(), predict_steps=1)
...
output_path = "Forecast/forecast_simple_exp_smoothing_2026.xlsx"
