...
def main():
    ...
    # 3.3 Disaggregation
    print("Disaggregatie naar Sizes...")
    # Calculate historical ratios
    size_agg = df.groupby(['channel_id', 'product_id', 'size'])['true_demand'].sum().reset_index()
    cp_agg = df.groupby(['channel_id', 'product_id'])['true_demand'].sum().reset_index().rename(columns={'true_demand':'total_cp'})
    ratios = size_agg.merge(cp_agg, on=['channel_id', 'product_id'])
    ratios['share'] = ratios['true_demand'] / ratios['total_cp']
    
    # Map reconciled CP to sizes
    cp_rec = nodes_df[nodes_df["level"] == 3][['stad', 'product', 'reconciled_2026', 'reconciled_2025', 'reconciled_2024']].rename(columns={'stad':'channel_id', 'product':'product_id'})
