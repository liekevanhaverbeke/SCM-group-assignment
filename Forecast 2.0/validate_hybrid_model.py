...
def run_hybrid_yearly(df, target_year):
    ...
    for name, gcols, method, lvl in level_configs:
        ...
        for _, row in combos.iterrows():
            ...
            if len(unique_years) > 1:
                # Test on the last 3 available years in history (like RESIDUAL_YEARS)
    ...
    city_total_agg = history.groupby(['channel_id'])['true_demand'].sum().reset_index().rename(columns={'true_demand':'total_city'})
    ratios = size_agg.merge(city_total_agg, on=['channel_id'])
    ratios['share'] = ratios['true_demand'] / ratios['total_city']
    ratios = ratios.drop(columns=['true_demand', 'total_city'])
...
def main():
    ...
    for year in years_to_validate:
        print(f"Running Hybrid MinT Reconciliation Logic for {year}...")
        forecast_df = run_hybrid_yearly(raw_demand, year)
    ...
    print("\n" + "="*50)
    print("YEARLY HYBRID MODEL PERFORMANCE (MinT Coherent + Top-Down SKU)")
    print("="*50)
    ...
    print("\n" + "="*50)
    print("TOP 5 WORST PRODUCTS (By MAE, Hybrid MinT 2020-2025)")
    print("="*50)
    print(product_metrics.sort_values(by='MAE', ascending=False).head(5).to_string(index=False))
    
    print("\n" + "="*50)
    print("CITY PERFORMANCE (Sorted by MAE, Hybrid MinT 2020-2025)")
    print("="*50)
    print(city_metrics.sort_values(by='MAE', ascending=False).to_string(index=False))
    
    output_file = 'Forecast/hybrid_validation_all_years.xlsx'
    ...
    print(f"\nDetailed hybrid validation reports saved to: {output_file}")
