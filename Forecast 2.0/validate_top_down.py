...
def lr_forecast(jaren, waarden, forecast_jaar):
    ...
    if len(jaren) < 2:
        return waarden[-1] if len(waarden) > 0 else 0.0
...
def load_proportions(target_year):
    ...
    if gender_prop.empty: return None, None, None, None
...
def generate_full_pipeline_forecast(target_year, raw_demand_df):
    """Calculates Linear Regression for City, then Disaggregates using prior averages."""
    # 1. Calculate LR City Forecast
    historical_city = raw_demand_df[raw_demand_df['season'] < target_year].groupby(['channel_id', 'season'])['true_demand'].sum().reset_index()
    
    if historical_city.empty:
        return pd.DataFrame()
        
    city_forecasts = []
    for channel in historical_city['channel_id'].unique():
        subset = historical_city[historical_city['channel_id'] == channel].sort_values('season')
        pred = lr_forecast(subset['season'].tolist(), subset['true_demand'].tolist(), target_year)
        city_forecasts.append({'channel_id': channel, 'lr_city_forecast': pred})
        
    df = pd.DataFrame(city_forecasts)
    
    # 2. Load Proportions
    gender_avg, price_avg, prod_avg, size_avg = load_proportions(target_year)
    if gender_avg is None:
        return pd.DataFrame()
        
    # 3. Disaggregate
    df = pd.merge(df, gender_avg, on='channel_id', how='left')
    df['forecast_gender'] = df['lr_city_forecast'] * (df['gender_pct'] / 100.0)
...
def main():
    ...
    for year in years_to_validate:
        print(f"Running Full Pipeline for {year} (LR training on <{year}, proportions on <{year})...")
        forecast_df = generate_full_pipeline_forecast(year, raw_demand)
    ...
    print("\n" + "="*50)
    print("YEARLY PIPELINE PERFORMANCE (LR City + Top-Down SKU)")
    print("="*50)
    ...
    print("\n" + "="*50)
    print("TOP 5 WORST PRODUCTS (By MAE, Combined 2020-2025)")
    print("="*50)
    print(product_metrics.sort_values(by='MAE', ascending=False).head(5).to_string(index=False))
    
    print("\n" + "="*50)
    print("CITY PERFORMANCE (Sorted by MAE, Combined 2020-2025)")
    print("="*50)
    print(city_metrics.sort_values(by='MAE', ascending=False).to_string(index=False))
    
    output_file = 'Forecast/full_pipeline_validation_all_years.xlsx'
    ...
    print(f"\nDetailed full pipeline reports saved to: {output_file}")
