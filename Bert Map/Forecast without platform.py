"""
VERENIGDE ARIMA VOORSPELLING (Alle Niveaus) - ZONDER PLATFORM KANAAL
- Consolideert ARIMA voorspellingen voor: Total, City, City/Product en City/Product/Size
- Exclusief 'Platform' kanaal vanwege afwijkende trends
...
# ── 1. Configuratie ──────────────────────────────────────────────────────────
# We gebruiken het absolute pad of gaan uit van de root van het project
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
INPUT_PATH = os.path.join(BASE_DIR, "True demand/True demand Simeon/True_Demand_Results.xlsx")
...
LEVELS = [
    {
        "name": "TOTAL",
        "group_cols": [],
        "output_xlsx": "Forecast/arima_total_no_platform.xlsx"
    },
    {
        "name": "CITY",
        "group_cols": ["channel_id"],
        "output_xlsx": "Forecast/arima_city_no_platform.xlsx"
    },
    {
        "name": "CITY_PRODUCT",
        "group_cols": ["channel_id", "product_id"],
        "output_xlsx": "Forecast/arima_city_product_no_platform.xlsx"
    },
    {
        "name": "CITY_PRODUCT_SIZE",
        "group_cols": ["channel_id", "product_id", "size"],
        "output_xlsx": "Forecast/arima_city_product_size_no_platform.xlsx"
    }
]
...
def run_level(df, config):
    ...
    # Zorg dat output_path relatief ten opzichte van de project root is
    output_path = os.path.join(BASE_DIR, config["output_xlsx"])
...
def main():
    print("="*80)
    print("UNIFIED ARIMA FORECASTING PIPELINE - NO PLATFORM")
    print("="*80)
    
    print(f"Data laden: {INPUT_PATH}...")
    if not os.path.exists(INPUT_PATH):
        print(f"FOUT: Bestand niet gevonden op {INPUT_PATH}")
        return
        
    df = pd.read_excel(INPUT_PATH, sheet_name=SHEET_NAME)
    
    # CRITIEKE STAP: Filter het 'Platform' kanaal eruit
    print(f"Origineel aantal records: {len(df)}")
    df = df[df['channel_id'] != 'Platform']
    print(f"Records na filteren 'Platform': {len(df)}")
    
    for config in LEVELS:
        run_level(df, config)
        
    print("\nAlle ARIMA niveaus (exclusief Platform) zijn succesvol verwerkt!")
